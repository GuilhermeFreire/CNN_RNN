...
def gen_description(g, checkpoint, num_chars, img_src, starting_char=h.SOS, top_n_chars=None):
	...
	with tf.Session() as sess:
		...
		state = None
		current_char = h.char_to_index[starting_char]
		chars = [current_char]
		...
		for i in range(num_chars):
			if state is not None:
				...
				# feed_dict={g["x"]: [[current_char]], g["init_state"]: img_vector}
				# feed_dict={g["x"]: [[current_char]], g["img"]: [img_src]}
				feed_dict={g["x"]: [[current_char]], g["init_state"]: img_vector}
			...
			if top_n_chars is not None:
				p = np.squeeze(preds)
				p[np.argsort(p)[:-top_n_chars]] = 0
				p = p / np.sum(p)
				current_char = np.random.choice(h.num_char, 1, p=p)[0]
			else:
				current_char = np.random.choice(h.num_char, 1, p=np.squeeze(preds))[0]

			chars.append(current_char)

	chars = map(lambda x: h.index_to_char[x], chars)
	print("".join(chars))
	saved_states = np.squeeze(np.array(saved_states))
	saved_states_json = saved_states.tolist()
	final_desc = "".join(chars)
	info = {"description": final_desc, "states": saved_states_json}
	json.dump(info, codecs.open("LSTM_states_chars_with_description.json", "w", encoding="utf-8"), separators =(",", ":"), sort_keys=True, indent=4)
	return "".join(chars)

# g = build_graph(cell_type="GRU", num_steps=68, state_size=512, batch_size=50, learning_rate=5e-4, training=True)
# losses = train_network(g, 100, num_steps=68, batch_size=50, save="saves/drawception/Model2_100_epochs_512_state")

t = h.load_test_dataset()
print(t[0][0].shape)

g = build_graph(cell_type="GRU", num_steps=1, batch_size=1, state_size=512)
generate_characters(g, "saves/drawception/Model2_100_epochs_512_state", 68, img_src = t[0][7], starting_char=h.SOS, top_n_chars=5)