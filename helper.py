...
class Helper:
	def __init__(self):
		...
		self.complimentary_path = "../drawception_dataset/Resized_DB/"
	...
	def load_single_description(self, index):
		...
		return np.lib.pad(np.asarray(list(map(self.convert_char_to_index, text))), (0, self.max_description_length - len(text)), "constant", constant_values=self.char_to_index[self.EOS])
	...
	def load_n_images(self, images):
		if(type(images) == int):
			...
			examples = images
		for i in examples:
			print(self.load_single_image(self.img_paths[i]))
			print("-------------------")
